...
class SantaClausFactorAnalysis:
    """Class to perform Santa Claus Factor Analysis on financial factors data."""
    ...
    def get_year_start_end_indices_factors(self):
        """Gets the start and end indices for each year in the factors data."""
        return Utilities.get_year_start_end_indices_from_dict(self.factors)

    def get_year_start_end_indices_trimmed_factors(self):
        """Gets the start and end indices for each year in the trimmed factors data."""
    ...
    def santa_claus_factor_analysis_sub_periods(self, december_or_santa_claus:str="santa_claus"):
        """Santa Claus Factor Analysis on sub periods defined by start and end indices of each year for each factor."""
    ...
    def get_data_santa_claus_factor_analysis_sub_periods(self, results_sub_periods, number_df:int=3, col_name:str="p-value (β1)", relative_ret_diff:bool=False):
        """To retrieve time series of p-values for all countries for each factor from results_sub_periods."""
    ...
    def get_extra_tests_data(self, results_sub_periods):
        """To retrieve time series of extra p-values for all countries for each factor from results_sub_periods."""
    ...
    def get_extra_tests_data_overall(self, results_overall):
        """To retrieve overall extra p-values for all countries for each factor from results_overall."""
    ...
    def get_p_values_corrected(self, results_sub_periods, december_or_santa_claus:str):
        """To retrieve time series of corrected p-values for all countries for each factor from results_sub_periods and extra tests."""
    ...
    def get_p_values_corrected_overall(self, december_or_santa_claus:str):
        """To retrieve overall corrected p-values for all countries for each factor from results_overall and extra tests."""
